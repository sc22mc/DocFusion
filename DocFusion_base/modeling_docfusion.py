...
from .configuration_docfusion import DocfusionConfig 
from .configuration_docfusion import DocfusionLanguageConfig
from .configuration_docfusion import DocfusionVisionConfig
...
_CONFIG_FOR_DOC = "DocfusionConfig"
...
class DocfusionLearnedPositionalEmbedding(nn.Embedding):
    """
    This module learns positional embeddings up to a fixed maximum size.
    """

    def __init__(self, num_embeddings: int, embedding_dim: int):
        # Docfusion is set up so that if padding_idx is specified then offset the embedding ids by 2
...
class DocfusionScaledWordEmbedding(nn.Embedding):
    """
...
class DocfusionAttention(nn.Module):
    ...
    def __init__(
        ...
        config: Optional[DocfusionLanguageConfig] = None,
    ):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
...
class DocfusionFlashAttention2(DocfusionAttention):
    """
    Docfusion flash attention module. This module inherits from `DocfusionAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # DocfusionFlashAttention2 attention does not support output_attentions
        if output_attentions:
            raise ValueError("DocfusionFlashAttention2 attention does not support output_attentions")
...
class DocfusionSdpaAttention(DocfusionAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "DocfusionModel is using DocfusionSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
FLORENCE2_ATTENTION_CLASSES = {
    "eager": DocfusionAttention,
    "sdpa": DocfusionSdpaAttention,
    "flash_attention_2": DocfusionFlashAttention2,
}


class DocfusionEncoderLayer(nn.Module):
    def __init__(self, config: DocfusionLanguageConfig):
        super().__init__()
...
class DocfusionDecoderLayer(nn.Module):
    def __init__(self, config: DocfusionLanguageConfig):
        super().__init__()
        self.embed_dim = config.d_model
...
class DocfusionLanguagePreTrainedModel(PreTrainedModel):
    config_class = DocfusionLanguageConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _keys_to_ignore_on_load_unexpected = ["encoder.version", "decoder.version"]
    _no_split_modules = [r"DocfusionEncoderLayer", r"DocfusionDecoderLayer"]
...
class DocfusionEncoder(DocfusionLanguagePreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`DocfusionEncoderLayer`].

    Args:
        config: DocfusionLanguageConfig
        embed_tokens (nn.Embedding): output embedding
    """

    def __init__(self, config: DocfusionLanguageConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_tokens = DocfusionScaledWordEmbedding(
        ...
        self.embed_positions = DocfusionLearnedPositionalEmbedding(
            config.max_position_embeddings,
            embed_dim,
        )
        # self.layers = nn.ModuleList([DocfusionEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def forward(
        ...
        elif input_ids is not None:  # 这里两个input都是为了获取形状
            input = input_ids
...
class DocfusionDecoder(DocfusionLanguagePreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`DocfusionDecoderLayer`]
    ...
    def __init__(self, config: DocfusionLanguageConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([DocfusionDecoderLayer(config) for _ in range(config.decoder_layers)])
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self._use_sdpa = config._attn_implementation == "sdpa"
...
class DocfusionLanguageModel(DocfusionLanguagePreTrainedModel):
    ...
    def __init__(self, config: DocfusionLanguageConfig):
        ...
        self.encoder = DocfusionEncoder(config, self.shared)
        self.decoder = DocfusionDecoder(config, self.shared)
    ...
    def forward(
        ...
        # different to other models, Docfusion automatically creates decoder_input_ids from
...
class DocfusionLanguageForConditionalGeneration(DocfusionLanguagePreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: DocfusionLanguageConfig):
        super().__init__(config)
        self.model = DocfusionLanguageModel(config)
    ...
    def adjusted_cross_entropy_loss(self, logits, targets, index_range=(50269, 51268), pad_token_index=1, sigma=1.5, kernel_size=9):
        """
        Computes the adjusted cross-entropy loss, ignoring the pad token.

        Parameters:
            logits: The model's output logits, shaped (batch_size, seq_len, vocab_size)
            targets: The ground truth labels, shaped (batch_size, seq_len)
            index_range: The range of indices to adjust, specified as (start_index, end_index), inclusive
            pad_token_index: The index of the pad token, used to ignore loss computation
            sigma: The standard deviation of the Gaussian distribution, controlling the decay of convolutional kernel weights
            kernel_size: The size of the convolutional kernel, which must be an odd number greater than 1

        Returns:
            loss: A scalar tensor representing the average loss for the batch
        """
        ...
        index_mask = torch.ones(vocab_size, dtype=torch.bool, device=logits.device)
        index_mask[start_idx:end_idx+1] = False 
        probs_copy[:, :, index_mask] = 0  

        positions = torch.arange(-(kernel_size // 2), kernel_size // 2 + 1, device=logits.device).float()
        kernel_weights = torch.exp(-0.5 * (positions / sigma) ** 2)
        kernel_weights = kernel_weights / kernel_weights[kernel_size // 2]
        kernel = kernel_weights.view(1, 1, -1) 

        padding_size = kernel_size // 2  
        padded_probs = F.pad(probs_copy, (padding_size, padding_size), mode='constant', value=0)
        conv_input = padded_probs.view(-1, 1, vocab_size + 2 * padding_size) 
        convolved = F.conv1d(conv_input, kernel, padding=0) 
        convolved = convolved.view(batch_size, seq_len, vocab_size) 

        index_mask = torch.zeros(vocab_size, dtype=torch.bool, device=logits.device)
        index_mask[start_idx:end_idx+1] = True  
        index_mask = index_mask.unsqueeze(0).unsqueeze(0)  
        probs = torch.where(index_mask, convolved, probs) 

        probs_flat = probs.view(-1, vocab_size)  
        targets_flat = targets.view(-1) 

        non_pad_mask = (targets_flat != pad_token_index)
        probs_flat = probs_flat[non_pad_mask]
        targets_flat = targets_flat[non_pad_mask]
        loss = F.nll_loss(torch.log(probs_flat), targets_flat, reduction='mean')
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss = self.adjusted_cross_entropy_loss(lm_logits, labels)
...
@dataclass
class DocfusionSeq2SeqLMOutput(ModelOutput):
    """
    Base class for Docfusion model's outputs that also contains : pre-computed hidden states that can speed up sequential
    decoding.
...
FLORENCE2_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`DocfusionConfig`] or [`DocfusionVisionConfig`]):
...
@add_start_docstrings(
    "The bare Docfusion Model outputting raw hidden-states without any specific head on top.",
    FLORENCE2_START_DOCSTRING,
)
class DocfusionPreTrainedModel(PreTrainedModel):
    config_class = DocfusionConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _skip_keys_device_placement = "past_key_values"
...
FLORENCE2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, image_size, image_size)):
            ...
            [`AutoImageProcessor`]. See [`CLIPImageProcessor.__call__`] for details ([]`DocfusionProcessor`] uses
...
class DocfusionVisionModel(DocfusionPreTrainedModel):
    def __init__(self, config: DocfusionVisionConfig):
        super().__init__(config)
...
class DocfusionVisionModelWithProjection(DocfusionPreTrainedModel):
    def __init__(self, config: DocfusionVisionConfig):
        super().__init__(config)
        assert config.model_type == 'davit', 'only DaViT is supported for now'
        self.vision_tower = DaViT.from_config(config=config)
...
class DocfusionForConditionalGeneration(DocfusionPreTrainedModel):
    def __init__(self, config: DocfusionConfig):
        super().__init__(config)
        ...
        language_model = DocfusionLanguageForConditionalGeneration(config=config.text_config)
    ...
    @replace_return_docstrings(output_type=DocfusionSeq2SeqLMOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, DocfusionSeq2SeqLMOutput]:
        r"""
        Args:
            labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
        ...
        >>> import requests
        >>> from transformers import AutoProcessor, DocfusionForConditionalGeneration

        >>> model = DocfusionForConditionalGeneration.from_pretrained("microsoft/Docfusion-large")
        >>> processor = AutoProcessor.from_pretrained("microsoft/Docfusion-large")
        ...
        return DocfusionSeq2SeqLMOutput(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            decoder_hidden_states=outputs.decoder_hidden_states,
